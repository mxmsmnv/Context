...
- Discontinued the standalone Context project.
- Integrated the complete Context feature set into the [Jigsaw](https://github.com/mxmsmnv/Jigsaw) super-module.
- Existing Context installations remain usable, but future development and maintenance continue in Jigsaw.

## [2.1.0] - 2026-07-23

### Added

- Added a recommended Squad gateway mode that reuses Squad's encrypted active key, default provider, and selected model.
- Added Squad status and model discovery to the existing `ContextAI` API.

### Changed

- New installations use Squad mode by default while direct OpenRouter, OpenAI, and custom OpenAI-compatible connections remain available.
- Context settings hide duplicate credential and model fields in Squad mode and link to the central Squad configuration.

## [2.0.1] - 2026-06-23

### Fixed

- Restored static module config compatibility by ensuring `ContextConfigFields` is loaded before `getModuleConfigInputfields()` uses it, preventing `Class "ProcessWire\\ContextConfigFields" not found` after upgrade.
